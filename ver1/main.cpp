...
#define NUM_MACHINE  1
 ...
 struct DEF_MACHINE {
     ...
     double SPw[15][15]  ; // Точки графика (S,Pw)
     double SMz[15][15]  ; // Точки графика (S,Mz)
 } ;
...
void TFormMain::CalcStandart(TObject *Sender)
{
  ...
  String str;
  int mach; // станок
  double D,z,ap,ae,vc,fz; //исходные данные
  double ak,fsi ; // Стружка
  double kpd,gamma0, kc11, mc ; // материал
  double tn,tvf,Q,thm,tkc,tNmot; // рассчитываемые параметры : ..., мощность
 ...
 /* Проверка на правильность ввода числовых значений
  ...
  if (D<=0.) { Application->MessageBox("Средняя толщина стружки \n D=0. \n Расчет прерван.","Предупреждение...",MB_OK); return ; }
  ...
  if (CheckBoxhm2->Checked==True) {
     ...
     if (ak<=0. || ak>=180.) {
       ...
       OutputMessage("Угол k - превышает лимит <=0 || >=180 -> установлен k=2");
     }
  ...
  if (gamma0<=0. || gamma0>=100.) {
    ...
    OutputMessage("Главный передний угол (гамма) - превышает лимит <=0 || >=100 -> установлен gamma=2");
  }
  /*Коэф материала kc11*/
  str=EditStKc11->Text;
  kc11=atof(str.c_str());
  if (kc11<=0.) {
    ...
    OutputMessage("Коэф материала kc11 - <=0 -> установлен kc11=700");
  }
  /*if (kc11==700.) {
  ...
  if (mc<=0. || mc>1.) {
    ...
    OutputMessage("Коэф материала kc11 - <=0 || >=1. -> установлен mc=0.25");
  }
  /*if (mc==0.25) {
    OutputMessage("Для более точного вычисления режима резания - выберите нужный материал");
  ...
  tkc=0.;
  if (thm<=0.) {
    OutputMessage("Расчет мощности резания->Средняя толщина стружки hm<=0");
...
void __fastcall TFormMain::FormCreate(TObject *Sender)
{
  ...
  MACHINE[1].machine_name[0]='\0';  strcpy(MACHINE[1].machine_name,"ВС-5ВС\0");
...
void __fastcall TFormMain::TabMachChange(TObject *Sender)
{
   ...
   s[0]='\0';
   sprintf(s,"n=%.0lf (об/мин)  F=%.0lf (мм/мин)  Nct=%.0lf (кВт) Mz=%.0lf (Hm)", \
...
void __fastcall TFormMain::PageControlMethodsChange(TObject *Sender)
{
  ...
  {
   case 0 :
    LabelB->Caption="Ширина резания, ap (мм) =";
    LabelT->Caption="Глубина резания, ae (мм) =";
    BaseVc->Visible=True;  LabelVc->Visible=True; //Enabled
    Basefz->Visible=True;  Labelfz->Visible=True;
   break ;
   default :
    LabelB->Caption="Ширина резания, B (мм) =";
    LabelT->Caption="Глубина резания, t (мм) =";
...
void __fastcall TFormMain::BitBtnSt5Click(TObject *Sender)
{
  ...
  double ak,fsi ; // Стружка
  double kpd,gamma0, kc11, mc ; // материал
  double tvf,thm,tkc,tNmot; // рассчитываемые параметры : ..., мощность
